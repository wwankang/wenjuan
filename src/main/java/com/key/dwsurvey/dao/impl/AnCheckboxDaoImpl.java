...
public class AnCheckboxDaoImpl extends BaseDaoImpl<AnCheckbox, String> implements AnCheckboxDao {
    ...
    public void findGroupStats(Question question) {
        ...
        Query query = this.getSession().createSQLQuery(sql).addScalar("qu_item_id", StandardBasicTypes.STRING).addScalar("count", StandardBasicTypes.INTEGER).setString(0, question.getId());
...
//		"and t1.belong_answer_id=t2.belong_answer_id GROUP BY yesno_answer,qu_item_id
    ...
    public List<DataCross> findStatsDataChart(Question question) {
        ...
        String sql = "select qu_item_id,count(*) count from t_an_checkbox where  visibility=1 and qu_id=? GROUP BY qu_item_id";
        String quId = question.getId();
        Query query = this.getSession().createSQLQuery(sql).addScalar("qu_item_id", StandardBasicTypes.STRING).addScalar("count", StandardBasicTypes.INTEGER).setParameter(0, quId);
        List<Object[]> list = query.list();
        List<QuCheckbox> quCheckboxs = question.getQuCheckboxs();
        for (QuCheckbox quCheckbox : quCheckboxs) {
