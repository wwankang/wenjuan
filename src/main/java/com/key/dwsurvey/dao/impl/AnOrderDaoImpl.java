...
public class AnOrderDaoImpl extends BaseDaoImpl<AnOrder, String> implements AnOrderDao {
	...
	public void findGroupStats(Question question) {
		...
		Query query = this.getSession().createSQLQuery(sql).addScalar("qu_row_id", StandardBasicTypes.STRING).addScalar("sumOrderNum", StandardBasicTypes.INTEGER).setString(0, question.getId());
